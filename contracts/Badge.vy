...
# @notice This is a basic ERC1155 and would be equivalent to Blueberry Club's Lab.sol contract.
# @dev The main customizations we would need to build are: A way to determine when someone is allowed to mint another badge. We could do this by having an off chain script that checks whenever an address locks cvx (or whatever other criteria we want to look at), then update the "allowed badge count" on the contract which would allow a user to mint another badge (could be expensive gas wise so maybe look into a more sophisticated way of doing this). Then, after minting, the user could "attach" the badge to a llama of their choice in a separate transaction.  Modified from @Doc-Pixel at https://github.com/vyperlang/vyper/blob/master/examples/tokens/ERC1155ownable.vy
# @author The Llamas
# @license MIT
#
...
MAX_URL_LENGTH: constant(uint256) = (
    MAX_URI_LENGTH + MAX_DYNURI_LENGTH + MAX_EXTENSION_LENGTH
)  # dynamic URI status
dynamicUri: bool
...
def __init__(
    name: String[128],
    symbol: String[16],
    uri: String[MAX_URI_LENGTH],
    contractUri: String[MAX_URI_LENGTH],
):
    """
...
@view
def balanceOfBatch(
    accounts: DynArray[address, BATCH_SIZE], ids: DynArray[uint256, BATCH_SIZE]
) -> DynArray[uint256, BATCH_SIZE]:  # uint256[BATCH_SIZE]:
    """
...
@external
def burnBatch(
    ids: DynArray[uint256, BATCH_SIZE], amounts: DynArray[uint256, BATCH_SIZE]
):
    """
...
def safeTransferFrom(
    ...
    assert (
        sender == msg.sender or self.isApprovedForAll[sender][msg.sender]
    ), "Caller is neither owner nor approved operator for this ID"
    assert (
        self.balanceOf[sender][id] > 0
