...
def create_pending_returns(auction_house, bidder_1, bidder_2):
    auction_house.create_bid(0, 100, sender=bidder_1, value="100 wei")
    auction_house.create_bid(0, 200, sender=bidder_2, value="200 wei")
...
def test_create_bid(auction_house_unpaused, alice):
    auction_house_unpaused.create_bid(0, 100, sender=alice, value="100 wei")
...
def test_create_bid_send_more_than_last_bid(auction_house_unpaused, alice, bob):
    ...
    auction_house_unpaused.create_bid(0, 1000, sender=bob, value="1000 wei")
...
def test_create_bid_value_too_low(auction_house_unpaused, alice):
    with ape.reverts("revert: Must send at least reservePrice"):
        auction_house_unpaused.create_bid(0, 1, sender=alice, value="1 wei")
...
def test_create_bid_not_over_prev_bid(auction_house_unpaused, alice, bob):
    ...
    with ape.reverts("revert: Must send more than last bid by min_bid_increment_percentage amount"):
        auction_house_unpaused.create_bid(0, 101, sender=bob, value="101 wei")
...
def test_create_bid_using_pending_returns(token, auction_house_unpaused, alice, bob):
    ...
    auction_house_unpaused.create_bid(0, 106, sender=bob, value="106 wei")
    ...
    auction_house_unpaused.create_bid(0, 125, sender=alice, value="25 wei")
...
def test_create_bid_using_pending_returns_from_previous_auction(
    ...
    auction_house_unpaused.create_bid(1, 100, sender=alice)
...
def test_create_bid_using_pending_returns_outbid(token, auction_house_unpaused, alice, bob):
    ...
    auction_house_unpaused.create_bid(0, 200, sender=bob, value="200 wei")
...
def test_create_bid_using_pending_returns_not_enough(auction_house_unpaused, alice, bob):
    ...
    with ape.reverts("revert: Does not have enough pending returns to cover remainder"):
        auction_house_unpaused.create_bid(0, 126, sender=alice, value="25 wei")
...
def test_settle_current_and_create_new_auction_with_bid_smart_contract_owner(
    ...
    auction_house_sc_owner.create_bid(0, 100, sender=alice, value="100 wei")
    ...
    split_recipient_after = split_recipient.balance
    assert auction_house_sc_owner.auction()["token_id"] == 1
    assert token.ownerOf(0) == alice
    assert deployer_balance_after == deployer_balance_before + 5
    assert split_recipient_after == split_recipient_before + 95
...
def test_bidder_outbids_prev_bidder(
    ...
    auction_house_unpaused.create_bid(0, 100, sender=alice, value="100 wei")
    auction_house_unpaused.create_bid(0, 1000, sender=bob, value="1000 wei")
    auction_house_unpaused.create_bid(0, 2000, sender=alice, value="2000 wei")
